...
new_user_id_exists=$(
  id ${APP_USER_ID} >/dev/null 2>&1
  echo $?
)

if [ "$new_user_id_exists" = "0" ]; then
  (echo >&2 "ERROR: APP_USER_ID $APP_USER_ID already exists - Aborting!")
  exit 1
fi

new_group_id_exists=$(
  getent group ${APP_GROUP_ID} >/dev/null 2>&1
  echo $?
)
if [ "$new_group_id_exists" = "0" ]; then
  (echo >&2 "ERROR: APP_GROUP_ID $APP_GROUP_ID already exists - Aborting!")
  exit 1
fi
# get current user and group in container
...
if [ "$old_user_id" != "${APP_USER_ID}" ]; then
  if [ "$old_user_exists" != "0" ]; then
    useradd ${APP_USER} -g ${APP_GROUP} # add user to created group
  fi
